...
Elm.WebSocket.make = function (_elm) {
   ...
   var io = $Native$WebSocket.io;
   var Socket = {ctor: "Socket"};
   return _elm.WebSocket.values = {_op: _op,io: io};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var messages = $Signal.mailbox("");
   var Disconnected = {ctor: "Disconnected"};
   var status = $Signal.mailbox(Disconnected);
   var main = A2($Signal.map,$Graphics$Element.show,status.signal);
   var Connected = {ctor: "Connected"};
   var connect = Elm.Native.Task.make(_elm).perform(A4($WebSocket.io,
   "ws://echo.websocket.org",
   A2($Signal.forwardTo,status.address,function (_p0) {    return Connected;}),
   messages.address,
   A2($Signal.forwardTo,status.address,function (_p1) {    return Disconnected;})));
   return _elm.Main.values = {_op: _op,Connected: Connected,Disconnected: Disconnected,messages: messages,status: status,main: main};
};
